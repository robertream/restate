...
use restate_types::invocation::{
    AttachInvocationRequest, AttachServiceRequest, GetInvocationOutputResponse, InvocationResponse,
    InvocationTermination, LinkCompletionNotification, LinkRequest, LinkResponse,
    NotifySignalRequest, PurgeInvocationRequest, RestartAsNewInvocationRequest,
    ResumeInvocationRequest, ServiceInvocation, UnlinkRequest, UnlinkResponse,
};
...
pub enum Command {
    ...
    // -- New PP <-> PP commands for linked services (Phase A)
    /// Link request: establish a LinkedFrom edge on the child node.
    LinkRequest(LinkRequest),
    /// Link response: confirm or reject the link establishment.
    LinkResponse(LinkResponse),
    /// Unlink request: parent partition → child partition to remove a LinkedFrom edge.
    ...
    /// Notify a parent that a child entity has completed (unified sink-based notification).
...
impl HasRecordKeys for Envelope {
    fn record_keys(&self) -> logs::Keys {
        match &self.command {
            ...
            Command::LinkRequest(req) => Keys::Single(req.partition_key()),
            Command::LinkResponse(resp) => Keys::Single(resp.partition_key()),
            Command::UnlinkRequest(req) => Keys::Single(req.partition_key()),
            Command::UnlinkResponse(resp) => Keys::Single(resp.partition_key()),
            Command::LinkCompletionNotification(notif) => Keys::Single(notif.partition_key()),
            Command::AttachServiceRequest(req) => Keys::Single(req.partition_key()),
        }
    }
}
